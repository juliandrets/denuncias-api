...
<div class="wrapper">
  ...
  <div class="content-wrapper">
          ...
          <div class="col-md-12">
            <div class="box">
              <div class="box-header with-border">
                <h3 class="box-title  pull-left">Sub Categorias cargadas</h3>
                ...
                @if($event = app('request')->input('event'))
                  <section class="notifications">
                    <div class="alert alert-success alert-dismissible">
                      ...
                      <i class="icon fa fa-check"></i> La subcategoria fue
                      @if($event == 'created')
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Borrar Subcategoria</h4>
          </div>
          <div class="modal-body">
            <p>Esta seguro de borrar la siguiente subcategoria? ({{ $categoria->name }}).</p>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Cerrar</button>
            <a href="/api/adm/subcategorias/{{ $categoria->id }}/delete"><button type="button" class="btn btn-danger">Borrar Sub Categoria</button></a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal fade" id="categoria-edit-{{ $categoria->id }}">
      <form role="form" method="POST" action="/api/adm/subcategorias/{{ $categoria->id }}/update">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span></button>
              <h4 class="modal-title">Editar subcategoria ({{ $categoria->name }})</h4>
            </div>
            <div class="modal-body">
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <select name="category_id" class="form-control">
                    @foreach($categories as $category)
                      <option value="{{ $category->id }}" @if($categoria->category->id == $category->id) selected @endif>{{ $category->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Cerrar</button>
              <button type="submit" class="btn btn-success">Editar Sub Categoria</button>
            </div>
          </div>
        </div>
      </form>
    ...
    <div class="modal fade" id="create">
      <form role="form" method="POST" action="/api/adm/subcategorias">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span></button>
              <h4 class="modal-title">Cargar nueva subcategoria</h4>
            </div>
            <div class="modal-body">
              <div class="box-body">
                <div class="form-group">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Cerrar</button>
              <button type="submit" class="btn btn-success">Cargar Sub Categoria</button>
            </div>
          </div>
        </div>
      </form>
